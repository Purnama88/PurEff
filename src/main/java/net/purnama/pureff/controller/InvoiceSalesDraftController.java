...
public class InvoiceSalesDraftController {
    ...
    public ResponseEntity<?> closeInvoiceSalesDraft(@RequestParam(value="id") String id) {
        ...
        if(numbering == null){
            ...
            numbering.setStatus(false);
            numbering.setLastmodified(Calendar.getInstance());
            numbering.setLastmodifiedby(invoicesalesdraft.getLastmodifiedby());
            numberingService.updateNumbering(numbering);
            return ResponseEntity.badRequest().body("Numbering is out of date");
        }
        ...
        else if(invoicesalesdraft.getPartner().getMaximumdiscount() >= 0){
            if(invoicesalesdraft.getPartner().getMaximumdiscount() < invoicesalesdraft.getDiscount_percentage()){
                return ResponseEntity.badRequest().body("Discount is exceeding partner's maximum discount");
            }
        }
        else if(invoicesalesdraft.getLastmodifiedby().getDiscount() >= 0){
            if(invoicesalesdraft.getLastmodifiedby().getDiscount() < invoicesalesdraft.getDiscount_percentage()){
        ...
        if(invoicesalesdraft.getPartner().getMaximumbalance() >= 0){
            
            if(invoicesalesdraft.getTotal_defaultcurrency() + 
                    invoicesalesdraft.getPartner().getBalance() > 
                    invoicesalesdraft.getPartner().getMaximumbalance()){
