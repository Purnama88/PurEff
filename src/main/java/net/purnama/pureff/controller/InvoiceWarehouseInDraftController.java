...
public class InvoiceWarehouseInDraftController {
    ...
    public ResponseEntity<?> closeInvoiceWarehouseInDraft(@RequestParam(value="id") String id) {
        ...
        if(!invoicewarehouseindraft.getLastmodifiedby().isDateforward()){
            if(invoicewarehouseindraft.getDate().getTime().getDate() > new Date().getDate()){
                return ResponseEntity.badRequest().body("You are not allowed to change date forward");
            }
        }
        
        if(!invoicewarehouseindraft.getLastmodifiedby().isDatebackward()){
            if(invoicewarehouseindraft.getDate().getTime().getDate() < new Date().getDate()){
    ...
    public ResponseEntity<?> getInvoiceWarehouseInDraftList(HttpServletRequest httpRequest,
        ...
        List<InvoiceWarehouseInDraftEntity> ls = invoicewarehouseindraftService.
                getInvoiceWarehouseInDraftList(itemperpage, page, sort, keyword, user, warehouse);
        return ResponseEntity.ok(ls);
    }
    ...
    public int countInvoiceWarehouseInDraftList(HttpServletRequest httpRequest,
        ...
        return invoicewarehouseindraftService.countInvoiceWarehouseInDraftList(keyword, user, warehouse);
    }
}
